...
- **`KNOWLEDGE.md`** — consolidated technical reference distilled from the corpus PDFs.
- **`SOFTWARE_INDEX.md`** — every named tool/library/slicer with license, status, fit, caveats.
...
A note on patents: anything that looks like it could plausibly read on a 5-axis-AM slicer/path-planner is flagged. **Patent claims are interpreted by lawyers, not by Claude.** Before shipping a commercial product, get a freedom-to-operate (FTO) opinion from a patent attorney in your jurisdiction. For research or open-source work, the risk profile is much lower but still nonzero.
...
3. Add Chinese-language literature — there is a large body of work in journals like *Computer Integrated Manufacturing Systems* and from Chinese universities (Huazhong UST, Tsinghua, SJTU) not searched here in Chinese.
